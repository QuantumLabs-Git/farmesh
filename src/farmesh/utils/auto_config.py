...
_CLASS_MAPPING = {}  # Populated by farmesh.models.* subpackages with register_model_classes()
...
class _AutoDistributedBase:
    ...
    def from_pretrained(cls, model_name_or_path: Union[str, os.PathLike, None], *args, **kwargs) -> PretrainedConfig:
        ...
        if config.model_type not in _CLASS_MAPPING:
            raise ValueError(f"FarMesh does not support model type {config.model_type}")

        proper_cls = getattr(_CLASS_MAPPING[config.model_type], cls._mapping_field)
        if proper_cls is None:
            raise ValueError(f"FarMesh does not have {cls.__name__} for model type {config.model_type}")
...
class DefaultRevisionMixin:
    """
    FarMesh only supports Falcon loaded in the new in-library format (transformers.FalconModel).
