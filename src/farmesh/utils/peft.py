...
from farmesh.server.block_utils import get_model_block, resolve_block_dtype
from farmesh.utils.convert_block import QuantType
from farmesh.utils.disk_cache import allow_cache_reads, allow_cache_writes, free_disk_space_for
...
def load_peft(
    ...
    *,
    ...
    # TODO: Check is it possible to add safetensors loading inside farmesh/server/from_pretrained.py and reuse it here
...
class AdapterContextMixin:
    ...
    def set_adapter(self, adapter_names) -> None:
        """
        In PEFT, this function makes the adapter trainable. However, in FarMesh environment this is not possible now. Thus,
...
def add_adapter_to_block(block, block_index, adapter_name, peft_config, peft_state_dict):
    assert peft_config["peft_type"] == "LORA", "FarMesh works only with LORA adapters"
