...
def convert_block(
    ...
    **kwargs,
    ...
    """
    Optimize a transformer block for use in a FarMesh server, apply tensor parallelism and/or LLM.8bit quantization
    ...
    if adapters:
        from farmesh.utils.peft import add_adapter_to_block, create_lora_adapter, load_peft
...
def quantize_module(model: nn.Module, *, quant_type: QuantType) -> nn.Module:
    # Import bitsandbytes only when necessary, so FarMesh runs on platforms not supported by bitsandbytes
