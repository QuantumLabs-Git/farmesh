...
from farmesh.client.routing import RemoteSequenceManager, maybe_log_traceback
from farmesh.data_structures import CHAIN_DELIMITER, ModuleUID, RemoteSpanInfo, RPCInfo
from farmesh.server.handler import TransformerConnectionHandler
from farmesh.utils.misc import DUMMY, DUMMY_INT64, is_dummy
from farmesh.utils.packaging import pack_args_kwargs
...
class InferenceSession:
    ...
    @property
    def last_token_id(self) -> Optional[torch.Tensor]:  # Backward compatibility with FarMesh < 2.1.0
        return self.output_ids[:, -1:] if self.output_ids is not None else None

    @last_token_id.setter
    def last_token_id(self, value: torch.Tensor):  # Backward compatibility with FarMesh < 2.1.0
