...
from farmesh.client.config import ClientConfig
from farmesh.client.routing.sequence_info import RemoteSequenceInfo
from farmesh.client.routing.spending_policy import NoSpendingPolicy
from farmesh.data_structures import ModuleUID, RemoteSpanInfo, ServerState
from farmesh.server.handler import TransformerConnectionHandler
from farmesh.utils.dht import get_remote_module_infos
...
class SequenceManagerConfig(ClientConfig):
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "farmesh.client.routing.SequenceManagerConfig has been moved to farmesh.ClientConfig. "
            "This alias will be removed in FarMesh 2.2.0+",
            DeprecationWarning,
            stacklevel=2,
        )
...
class RemoteSequenceManager:
    ...
    def get_request_metadata(
        ...
        :param args_structure: the structure of flattened tensors from pack_args_kwargs in farmesh.utils.packaging
...
class MissingBlocksError(RuntimeError):
    def __init__(self, block_indices: Union[int, Sequence[int]]):
        super().__init__(
            ...
            f"You can check the public swarm's state at https://health.farmesh.dev "
            f"If there are not enough servers, please connect your GPU: "
            f"https://github.com/QuantumLabs-Git/farmesh#connect-your-gpu-and-increase-farmesh-capacity "
        )
